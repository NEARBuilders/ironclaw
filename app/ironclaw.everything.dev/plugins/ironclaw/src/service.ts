...
export class IronclawService {
  ...
  pauseAutomation(id: string): Effect.Effect<void, Error> {
    return Effect.tryPromise({
      try: () =>
        this.request<void>("POST", `/api/webchat/v2/automations/${encodeURIComponent(id)}/pause`),
      catch: (error: unknown) =>
        new Error(
          `Failed to pause automation: ${error instanceof Error ? error.message : String(error)}`,
        ),
    });
  }

  resumeAutomation(id: string): Effect.Effect<void, Error> {
    return Effect.tryPromise({
      try: () =>
        this.request<void>("POST", `/api/webchat/v2/automations/${encodeURIComponent(id)}/resume`),
      catch: (error: unknown) =>
        new Error(
          `Failed to resume automation: ${error instanceof Error ? error.message : String(error)}`,
        ),
    });
  }

  deleteAutomation(id: string): Effect.Effect<void, Error> {
    return Effect.tryPromise({
      try: () =>
        this.request<void>("DELETE", `/api/webchat/v2/automations/${encodeURIComponent(id)}`),
      catch: (error: unknown) =>
        new Error(
          `Failed to delete automation: ${error instanceof Error ? error.message : String(error)}`,
        ),
    });
  }

  setSkillAutoActivate(name: string, enabled: boolean): Effect.Effect<void, Error> {
    return Effect.tryPromise({
      try: () =>
        this.request<void>(
          "POST",
        ...
        new Error(
          `Failed to set skill auto-activate: ${error instanceof Error ? error.message : String(error)}`,
        ),
    });
  }

  setAutoActivateLearned(enabled: boolean): Effect.Effect<void, Error> {
    return Effect.tryPromise({
      try: () =>
        this.request<void>("POST", "/api/webchat/v2/skills/auto-activate-learned", { enabled }),
      catch: (error: unknown) =>
        new Error(
          `Failed to set auto-activate learned: ${error instanceof Error ? error.message : String(error)}`,
        ),
    });
  }

  getToolSettings(): Effect.Effect<{ data: ToolSetting[] }, Error> {
    return Effect.tryPromise({
      try: async () => {
        const raw: any = await this.request("GET", "/api/webchat/v2/settings/tools");
        const settings: ToolSetting[] = (raw.settings ?? []).map((s: any) => ({
        ...
        new Error(
          `Failed to get tool settings: ${error instanceof Error ? error.message : String(error)}`,
        ),
    });
  }
  ...
  setToolPermission(capabilityId: string, state: string): Effect.Effect<void, Error> {
    return Effect.tryPromise({
      try: () =>
        this.request<void>(
          "POST",
        ...
        new Error(
          `Failed to set tool permission: ${error instanceof Error ? error.message : String(error)}`,
        ),
    });
  }
