  ...
  createRouter: (context, builder) => {
    ...
    return {
      ...
      automations: {
        ...
        pause: builder.automations.pause.use(requireAuth).handler(
          ri(async (svc, input) => {
            await Effect.runPromise(svc.pauseAutomation(input.id));
            return { success: true };
          }),
        ),

        resume: builder.automations.resume.use(requireAuth).handler(
          ri(async (svc, input) => {
            await Effect.runPromise(svc.resumeAutomation(input.id));
            return { success: true };
          }),
        ),

        delete: builder.automations.delete.use(requireAuth).handler(
          ri(async (svc, input) => {
            await Effect.runPromise(svc.deleteAutomation(input.id));
            return { success: true };
          }),
        ),
      },
      ...
      skills: {
        ...
        autoActivate: builder.skills.autoActivate.use(requireAuth).handler(
          ri(async (svc, input) => {
            await Effect.runPromise(svc.setSkillAutoActivate(input.name, input.enabled));
            return { success: true };
          }),
        ),

        autoActivateLearned: builder.skills.autoActivateLearned.use(requireAuth).handler(
          ri(async (svc, input) => {
            await Effect.runPromise(svc.setAutoActivateLearned(input.enabled));
            return { success: true };
          }),
        ),
      },
      ...
      settings: {
        tools: {
          list: builder.settings.tools.list
            .use(requireAuth)
            .handler(r((svc) => Effect.runPromise(svc.getToolSettings()))),

          set: builder.settings.tools.set.use(requireAuth).handler(
            ri(async (svc, input) => {
              await Effect.runPromise(svc.setToolAutoApprove(input.enabled));
              return { success: true };
            }),
          ),

          setPermission: builder.settings.tools.setPermission.use(requireAuth).handler(
            ri(async (svc, input) => {
              await Effect.runPromise(svc.setToolPermission(input.capabilityId, input.state));
              return { success: true };
            }),
          ),
        },
